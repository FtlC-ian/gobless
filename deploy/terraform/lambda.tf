...
resource "aws_lambda_function" "gobless" {
  ...
  environment {
    variables = {
      ...
      GOBLESS_CA_DEFAULT_TTL                = tostring(var.max_ttl_seconds)
      ...
      GOBLESS_PRINCIPAL_ALLOWED_CERT_TYPES  = join(",", var.allowed_cert_types)
      GOBLESS_PRINCIPAL_ENFORCE_IAM_BINDING = tostring(var.enforce_iam_binding)
      GOBLESS_PRINCIPAL_EXPECTED_ACCOUNT_ID = var.expected_account_id
    }
  }
